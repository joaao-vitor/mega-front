    ...
    setup(props) {
        ...
        const submitGame = async (game) => {
            ...
            try {
                ...
                const { data } = await axios.put(
                    `${import.meta.env.VITE_API_URL}/jogo/update/${props.id}`,
                    game
                )
            } catch (error) {
                imgGame.value = null
        ...
        const getGame = async () => {
            const { data: gameData } = await axios.get(
                `${import.meta.env.VITE_API_URL}/jogo/${props.id}`
            )

            gameName.value = gameData.nome
            gameGenre.value = gameData.genero
            let data = new Date(gameData.data_lancamento)
            let dataFormatada =
                data.getFullYear() +
                '-' +
                `${
            ...
            dataLancamento.value = dataFormatada
            description.value = gameData.descricao
            selectedDevs.value = gameData.empresas
            imgUrl.value = gameData.imgurl
        }
        ...
        onMounted(async () => {
            await getGame()
            for (const dev of selectedDevs.value) {
                gameDevs.value = gameDevs.value.filter(
                    (d) => dev.empresaid !== d.empresaid
                )
            }
        })
